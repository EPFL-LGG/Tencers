#ifndef TENCER_KNITRO_PROBLEM_HH
#define TENCER_KNITRO_PROBLEM_HH

#if HAS_KNITRO

#include <DesignOptimization/knitro.hh>
...
struct OptKnitroProblem : public KnitroProblem<OptKnitroProblem<OptimizationObject>> {
    ...
    OptKnitroProblem(OptimizationObject &obj_opt, std::vector<int> &fixed_rest_vars)
        : Base(obj_opt.numParams(), 0), m_obj_opt(obj_opt)
    {
        this->setObjType(KPREFIX(OBJTYPE_GENERAL));
        this->setObjGoal(KPREFIX(OBJGOAL_MINIMIZE));

        // Get the bounds for the design parameters
        std::vector<double> xLoBounds = m_obj_opt.get_vars_lower_bounds();
        std::vector<double> xUpBounds = m_obj_opt.get_vars_upper_bounds();

        for (int i : fixed_rest_vars){
            double p = m_obj_opt.params()[i];
            xLoBounds[i] = p;
            xUpBounds[i] = p;
        }

        // Map std::numeric_limits<Real>::infinity() -> KPREFIX(INFBOUND)
        auto infMapKnitro = [](Real x) { return (x == std::numeric_limits<Real>::infinity()) ? KPREFIX(INFBOUND) : x; };
        for (auto& x : xLoBounds) x = infMapKnitro(x);
        for (auto& x : xUpBounds) x = infMapKnitro(x);

        // Set the bounds
        this->setVarLoBnds(xLoBounds);
        this->setVarUpBnds(xUpBounds);
    }
    ...
    double evalFC(const double *x,
                        ...
                        double * objGrad,
                        double * /* jac */) {
        const size_t np = m_obj_opt.numParams();
        auto params = Eigen::Map<const Eigen::VectorXd>(x, np);
        Real val = m_obj_opt.J(params);

        auto g = m_obj_opt.gradp_J(params);
    ...
    int evalHessVec(const double *x, double sigma, const double * /* lambda */,
        ...
        auto result = m_obj_opt.apply_hess(params, delta_p, sigma);
        Eigen::Map<Eigen::VectorXd>(hessVec, np) = result;
        return 0; // indicate success
    }

private:
    OptimizationObject &m_obj_opt;
};

template<class OptimizationObject>
struct OptKnitroNewPtCallback : public NewPtCallbackBase {
    OptKnitroNewPtCallback(OptimizationObject &obj_opt, std::function<void()> update_viewer)
        : NewPtCallbackBase(), m_update_viewer(update_viewer), m_obj_opt(obj_opt) { }

    virtual int operator()(const double *x) override {
        const size_t np = m_obj_opt.numParams();
        m_obj_opt.newPt(Eigen::Map<const Eigen::VectorXd>(x, np));
        //std::cout << "update viewer" << std::endl;
        try {
            m_update_viewer();
        }
        catch (...) {
            std::cout << "VIEWER EXCEPTION" << std::endl;
        }
...
void configureKnitroSolver(KnitroSolver &solver, int num_steps, Real trust_region_scale, Real optimality_tol) {
    ...
    solver.setParam(KPREFIX(PARAM_OPTTOLABS), optimality_tol);
    solver.setParam(KPREFIX(PARAM_OUTLEV), KPREFIX(OUTLEV_ALL));

    solver.setParam(KPREFIX(PARAM_XTOL), 1e-9);
    // solver.setParam(KPREFIX(PARAM_OPTTOLABS), optimality_tol);   // TODO: temp hack

    // std::cout << "Knitro PARAM_XTOL="   << solver.getDoubleParam(KPREFIX(PARAM_XTOL)) << std::endl;
    // std::cout << "Knitro PARAM_OPTTOL=" << solver.getDoubleParam(KPREFIX(PARAM_OPTTOL)) << std::endl;
    // std::cout << "Knitro PARAM_OPTTOLABS=" << solver.getDoubleParam(KPREFIX(PARAM_OPTTOLABS)) << std::endl;
}

template<class OptimizationObject>
int optimize(OptimizationObject &obj_opt, OptAlgorithm alg, size_t num_steps,
              Real trust_region_scale, Real optimality_tol, std::function<void()> &update_viewer, std::vector<int> &fixed_rest_vars) {
    OptKnitroProblem<OptimizationObject> problem(obj_opt,fixed_rest_vars);

    std::vector<Real> x_init(obj_opt.numParams());
    Eigen::Map<Eigen::VectorXd>(x_init.data(), x_init.size()) = obj_opt.params();
    problem.setXInitial(x_init);

    OptKnitroNewPtCallback<OptimizationObject> callback(obj_opt, update_viewer);
...
int optimize(OptimizationObject &/* um_opt */, OptAlgorithm /* alg */, size_t /* num_steps */,
              Real /* trust_region_scale */, Real /* optimality_tol */, std::function<void()> &/* update_viewer */, double /* min_stiffness */) {
